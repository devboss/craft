...
class HttpRequestService extends \CHttpRequest
{
	...
	public function validateCsrfToken($event)
	{
		if ($this->getIsPostRequest() || $this->getIsPutRequest() || $this->getIsPatchRequest() || $this->getIsDeleteRequest())
		{
			...
			{
                Craft::log('[CSRF] Cookie value:');
                Craft::log($csrfCookie->value);
                
                Craft::log('[CSRF] Request value:');
                Craft::log($tokenFromPost);
				...
				{
                    Craft::log('[CSRF] Cookie and request token match');
	...
	protected function csrfTokenValidForCurrentUser($token)
	{
		...
		{
            Craft::log('[CSRF] User doesn\'t exist');
            $sesh = isset($_SESSION) ? json_encode($_SESSION) : '{}';
            Craft::log("[CSRF] Session: {$sesh}");
